...
app.controller('userVisitController',function($scope,$window, $location){
    ...
    $scope.updateWeek = function(){
        ...
        for (let i = 0; i <= 6; i++) {
            //let first = Math.abs(curr.getDate() - curr.getDay() + i);
    ...
    function getVisit (index){
        ...
        UservisitTrackingController.userPopulated($scope.currentDateMap[index].date,$scope.userId,$scope.position.lat,$scope.position.lng,function(result,event){
            ...
            if(!$scope.dayVisitPlanId && result.visitList.length > 0){
                Swal.fire({title:'Welcome',text:'Please Start Your Day!'},function(isConfirm) {
    ...
    $scope.changeAmendVisit = function(){
        ...
        if(newDate < existingDate){
            $scope.amendVisitHandling = false;
            Swal.fire({title:'Date Warning',text:'Please choose future date.'},function(isConfirm) {
                if(isConfirm){
                    $scope.amendVisitHandling = true;
                    $scope.$apply();
                }
    ...
    $scope.startDay = function(){
        ...
        UservisitTrackingController.createVisitPlanAcrossUser($scope.position.lat,$scope.position.lng,$scope.userId, $scope.selectedDate
            ,function(result,event){
                if(result){
                    ...
                    Swal.fire({title:'VISIT PLAN',text:result.message},function(isConfirm) { 
                        $scope.$apply();
                    });
            ...
            Swal.fire({title:'Warning',text:'You still have pending visit to be closed, do you still wanna end your day?', showCancelButton: true}).then(result => {
                ...
                if(result.isConfirmed) {
                    if($scope.dayVisitPlanId) {
                        UservisitTrackingController.endDay($scope.position.lat,$scope.position.lng, $scope.userId, $scope.dayVisitPlanId
                        ,function(result,event){
                            if(result) {
                                $scope.disableTodayVisit = true;
            					$scope.dayHasEnded = true;
                        ...
                        Swal.fire({title:'VISIT PLAN',text:'Something went wrong, please contact system administrator.'},function(isConfirm) { 
                        });
                    }
                }
            });
...
function initMap(markerLocationList, repLat, repLong) {
    ...
    if(repLat && repLong) {
        markStore(map, {name:'Your Location', location: new google.maps.LatLng(repLat, repLong)}, getMarkerIconURL('CURRENTLOC'));
    ...
    for(var key in markerLocationList){
        if(key != 'remove'){
            markStore(map, {name:markerLocationList[key].store_name, location: new google.maps.LatLng(markerLocationList[key].lat, markerLocationList[key].lng)}, getMarkerIconURL(markerLocationList[key].status));
...
function drawRoute(originAddress, destinationAddress, _waypoints) {    
    ...
    directionsService.route(_request, /* customDirectionsRenderer */ function (_response, _status) {   
